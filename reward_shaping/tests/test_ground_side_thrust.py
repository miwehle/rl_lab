...
from reward_shaping import RewardShapingEnv
...
class TestRewardShapingEnv:
    def test_step_penalizes_side_thrust_when_lander_is_awake_on_both_legs(self) -> None:
        env = RewardShapingEnv(FakeLanderEnv(both_legs_grounded=True, awake=True), ground_side_thrust_penalty=2.5)
    ...
    def test_step_keeps_reward_when_only_one_leg_has_ground_contact(self) -> None:
        ...
        env = RewardShapingEnv(base_env, ground_side_thrust_penalty=2.5)
    ...
    def test_step_keeps_reward_when_lander_is_not_awake(self) -> None:
        env = RewardShapingEnv(FakeLanderEnv(both_legs_grounded=True, awake=False), ground_side_thrust_penalty=2.5)
    ...
    def test_init_rejects_negative_penalty(self) -> None:
        with pytest.raises(ValueError, match="ground_side_thrust_penalty must be >= 0"):
            RewardShapingEnv(FakeLanderEnv(both_legs_grounded=True, awake=True), ground_side_thrust_penalty=-0.1)
