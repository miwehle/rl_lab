...
def compute_activity_layout(
    ...
    *,
    ...
    h2_nodes = _hidden2_nodes(rollouts.h2, h2_output_specificity, output_layer_nodes)
...
def target_specificity(relevance: np.ndarray) -> np.ndarray:
...
def output_nodes(q_values: np.ndarray) -> tuple[Node, ...]:
...
def input_nodes(observations: np.ndarray) -> tuple[Node, ...]:
    return tuple(
