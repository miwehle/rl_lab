...
def test_render_trace_step_3d_html_passes_window_state_to_pyvista_renderer(monkeypatch, tmp_path):
    import nn_viz.trace as trace_module

    trace_path = tmp_path / "trace.npz"
    output_path = tmp_path / "step_3d.html"
    np.savez(
        trace_path,
        steps=np.array([0, 1], dtype=np.int64),
        observations=np.array([[2.0], [4.0]], dtype=np.float32),
    ...
    def fake_render_state_html(layout, state, path, **kwargs):
        captured["layout"] = layout
        captured["state"] = state
        captured["path"] = path
        captured["kwargs"] = kwargs
    ...
    monkeypatch.setattr(trace_module, "render_state_html", fake_render_state_html)

    result = render_trace_step_3d_html(
        trace_path,
        minimal_layout(),
        output_path,
        step=1,
