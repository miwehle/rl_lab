...
def _px(value: float, render_scale: int) -> int:
    return round(value * render_scale)


def draw_wind_indicator(
    surface, font, wind: WindState, overlay: LanderOverlay, origin: tuple[int, int], *, render_scale: int = 1
) -> None:
    if wind.acceleration is None:
        return

    px = lambda value: _px(value, render_scale)
    acceleration = wind.acceleration
    color = _force_color(abs(acceleration), WIND_COLOR_STOPS)
    _draw_horizontal_force_arrow(
        surface, origin, acceleration, scale=_WIND_ACCEL_SCALE, color=color, overlay=overlay, render_scale=render_scale
    )
    _draw_centered_text(
        surface, font, f"wind {acceleration:+.1f} m/s²", (origin[0], origin[1] + px(13)), color, overlay
    )


def draw_kick_indicator(
    surface, font, env_state: EnvState, overlay: LanderOverlay, origin: tuple[int, int], *, render_scale: int = 1
) -> None:
    ...
    length = px(14 + min(delta_v / _KICK_DELTA_V_SCALE, 1.0) * 96)
    screen_direction = _normalized((env_state.initial_kick_delta_v[0], -env_state.initial_kick_delta_v[1]))
    length = _fit_vector_length(surface, origin, screen_direction, length, margin=px(6))
    end = (origin[0] + screen_direction[0] * length, origin[1] + screen_direction[1] * length)
    _draw_shadowed_line_arrow(surface, origin, end, color, overlay, width=px(3), render_scale=render_scale)
    _draw_centered_text(
        surface, font, f"kick {delta_v:.1f} m/s", (origin[0], origin[1] - px(20)), color, overlay
    )


def disturbance_vector_origin(surface, *, render_scale: int = 1) -> tuple[int, int]:
    return surface.get_width() // 2, _px(_DISTURBANCE_VECTOR_ORIGIN_Y, render_scale)


def draw_turbulence_indicator(
    surface, font, turbulence: TurbulenceState, lander_center: tuple[int, int] | None, overlay: LanderOverlay, *, render_scale: int = 1
) -> None:
    ...
    margin = px(26)
    if lander_center[1] < px(80):
        center = (_clamp(lander_center[0] + px(110), margin, surface.get_width() - margin), px(62))
    else:
        center = (
            _clamp(lander_center[0] + px(92), margin, surface.get_width() - margin),
            _clamp(lander_center[1] - px(38), margin, surface.get_height() - margin),
        )
    _draw_torque_arrow(
        surface,
        center,
    ...
    _draw_centered_text(
        ...
        (center[0], center[1] + px(18)),
        color,
        overlay,
    )


def _draw_centered_text(surface, font, line: str, center: tuple[int, int], color: RGB, overlay: LanderOverlay) -> None:
...
def _fit_vector_length(surface, origin: tuple[int, int], direction: tuple[float, float], length: float, *, margin: int) -> float:
    max_length = length
    if direction[0] > 0:
...
def _draw_horizontal_force_arrow(
    surface, origin: tuple[int, int], value: float, *, scale: float, color: RGB, overlay: LanderOverlay, render_scale: int = 1
) -> None:
    px = lambda value: _px(value, render_scale)
    length = px(14 + min(abs(value) / scale, 1.0) * 96)
...
def _draw_torque_arrow(
    surface, center: tuple[int, int], value: float, *, scale: float, color: RGB, shadow_color: RGB, render_scale: int = 1
) -> None:
    import pygame
    ...
    _draw_arrow_head(surface, tip, tangent, color, size=px(8))


def _draw_shadowed_line_arrow(
    surface,
    ...
    *,
    ...
    _draw_line_arrow(surface, start, end, color, width=width, render_scale=render_scale)
...
def _draw_line_arrow(
    ...
    *,
    ...
    if direction_length:
        tip = (
            end[0] + direction[0] / direction_length * px(2),
            end[1] + direction[1] / direction_length * px(2),
        )
    else:
        tip = end
    _draw_arrow_head(surface, tip, direction, color, size=px(8))
