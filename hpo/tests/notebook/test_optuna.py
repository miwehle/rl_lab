...
def test_db_path_returns_colab_paths() -> None:
    assert db_path("study", True) == (Path("/content/drive/MyDrive/rl_lab/hpo/study.db"))
    assert db_path("study", google_drive=True) == (Path("/content/drive/MyDrive/rl_lab/hpo/study.db"))
    assert db_path("study.db") == Path("/content/rl_lab/hpo/study.db")
    assert db_path("study", folder="rl_lab/hpo/runs") == (Path("/content/rl_lab/hpo/runs/study.db"))


def test_neighbors_returns_value_plus_direct_neighbors() -> None:
    assert neighbors(10_000, [2_500, 5_000, 10_000, 20_000]) == [5_000, 10_000, 20_000]
