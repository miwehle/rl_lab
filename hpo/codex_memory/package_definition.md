...
The active `solar_system_lander_10d_elise_stp` best-eval checkpoint folder in Drive is the converted/new-format folder and is used by the video notebook. Old not-directly-readable checkpoint folders were moved to `G:\Meine Ablage\rl_lab\hpo\best_checkpoints\_archive\old_checkpoints_in_old_format`.

For emergency preservation, copying the whole local checkpoint directory to Drive is acceptable when it is tiny.

Best-eval checkpoints in Drive are hard-won artifacts. Do not delete, overwrite, rename, or replace checkpoint archives unless the user explicitly asks for that exact destructive action.
